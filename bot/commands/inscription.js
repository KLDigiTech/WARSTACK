// ============================================
// COMMANDE — inscription.js
// Inscription au tournoi en cours
// ============================================
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // Vérifie qu'il y a un tournoi actif
    ...
    if (tournoi.max_players) {
      ...
      if (count >= tournoi.max_players) {
        return interaction.editReply({
          content: `❌ Le tournoi est complet (${tournoi.max_players} joueurs max).`
        });
      }
    }
    ...
    if (existing) {
      return interaction.editReply({
        content: `⚠️ Tu es déjà inscrit au tournoi **${tournoi.name}**.`
      });
    }
