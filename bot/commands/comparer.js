...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option.setName('joueur1').setDescription('Premier joueur').setRequired(true)
    )
    .addUserOption(option =>
      option.setName('joueur2').setDescription('Deuxième joueur').setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    const [data1, data2] = await Promise.all([
      getPlayerSnapshot(user1.id),
      getPlayerSnapshot(user2.id),
    ]);

    if (!data1?.snapshot) {
      return interaction.editReply({ content: `❌ **${user1.username}** n'a pas de stats disponibles.` });
    }
    if (!data2?.snapshot) {
      return interaction.editReply({ content: `❌ **${user2.username}** n'a pas de stats disponibles.` });
    }

    const s1 = data1.snapshot;
    const s2 = data2.snapshot;
    const score1 = calcScore(s1);
    const score2 = calcScore(s2);
    const div1 = getDivision(score1);
    const div2 = getDivision(score2);
    const n1 = data1.player.username || user1.username;
    const n2 = data2.player.username || user2.username;
