// ============================================
// COMMANDE — setup.js
// Poste le message de bienvenue épinglé
// ============================================
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const btnDashboard = new ButtonBuilder()
      .setLabel('🖥️ Dashboard Admin')
      .setURL('https://warstack.netlify.app')
      .setStyle(ButtonStyle.Link);

    const btnSite = new ButtonBuilder()
      .setLabel('🌐 Classement Live')
      .setURL('https://kldigitech.github.io/pof-bf6/Pages/tournoi.html')
      .setStyle(ButtonStyle.Link);

    const btnDiscord = new ButtonBuilder()
      .setLabel('💬 Serveur Discord')
      .setURL('https://discord.gg/tonlien')
      .setStyle(ButtonStyle.Link);

    const row = new ActionRowBuilder()
      .addComponents(btnDashboard, btnSite, btnDiscord);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: '📋 Commandes', value: '`/link` `/stats` `/classement` `/mvp` `/profil` `/comparer`' },
      )
      .setFooter({ text: 'WARSTACK • PöF BF6 Tournament' })
      .setTimestamp();

    // Poste dans le salon actuel
    ...
    await interaction.reply({
      content: '✅ Message de bienvenue posté et épinglé !',
      ephemeral: true
    });
  }
};