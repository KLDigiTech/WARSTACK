// ============================================
// SERVICE — rankup.js
// Détection montée de division WARSTACK
// ============================================

const { EmbedBuilder } = require('discord.js');

// Ordre des divisions pour comparer
...
function checkRankUp(oldDiv, newDiv) {
  ...
  if (newIndex > oldIndex) {
    return { rankUp: true, from: oldDiv, to: newDiv };
  }
  return { rankUp: false };
}
  ...
  try {
    ...
    if (user) {
      const dmEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `Tu viens de passer de **${from.emoji} ${from.name}** à **${to.emoji} ${to.name}**.\n\n` +
      ...
      await user.send({ embeds: [dmEmbed] }).catch(() => {
        console.log(`⚠️ DM impossible pour ${user.username} (DMs fermés)`);
      });
    }

    // Post dans salon #annonces-mvp ou #classement
    const channel =
    ...
    if (channel) {
      const channelEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `<@${discordId}> vient de passer **${from.emoji} ${from.name}** → **${to.emoji} ${to.name}** !`
        )
        .setColor(0xFF6600)
        .setFooter({ text: 'WARSTACK • Battlefield 6' })
        .setTimestamp();
